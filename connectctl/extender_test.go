...
// The extender names are derived from --api_url the way the sdk derives a
// space's service names (F1, G4).
func TestExtenderNamesFromApiUrl(t *testing.T) {
	hostCases := []struct {
		...
		{"https://api.example.com", "api.example.com", "example.com", "extender.example.com"},
		{"https://g2-api.example.com/secret", "g2-api.example.com", "example.com", "g2-extender.example.com"},
		{"http://api.space.example:8080", "api.space.example", "space.example", "extender.space.example"},
		{"http://127.0.0.1:8080", "127.0.0.1", "127.0.0.1", ""},
		{"http://[::1]:8080", "::1", "::1", ""},
		{"https://example.com", "example.com", "example.com", ""},
		{"https://localhost:8080", "localhost", "localhost", ""},
	}
	for _, c := range hostCases {
		apiHost, err := extenderApiHost(c.apiUrl)
		if err != nil {
			t.Errorf("extenderApiHost(%q) = %v", c.apiUrl, err)
			continue
		}
		if apiHost != c.apiHost {
			t.Errorf("extenderApiHost(%q) = %q, want %q", c.apiUrl, apiHost, c.apiHost)
		}
		if networkHost := extenderNetworkHost(apiHost); networkHost != c.networkHost {
			t.Errorf("extenderNetworkHost(%q) = %q, want %q", apiHost, networkHost, c.networkHost)
		}
		if dnsName := extenderServiceHostName(c.apiUrl, "extender"); dnsName != c.dnsName {
			t.Errorf("extenderServiceHostName(%q) = %q, want %q", c.apiUrl, dnsName, c.dnsName)
		}
	}
	if _, err := extenderApiHost("not a url"); err == nil {
