...
| --- | --- | --- |
| `ReliableLaneProvenRecovery`, the lane rule | campaign | 8 of 120 wedges with it on against 0 of 120 with it off, pooled over four cells and three independently built arms; Fisher two-sided 0.0069. A wedge is a run over 100 seconds whose write-to-defer ratio is below 1 and at least 80 per cent of whose windows are dead, which separates perfectly: every wedge reads 0.01 to 0.25 and 0.90 to 0.99, and every one is rule-on. By the cruder 100-second proxy alone it is 8 of 120 against 1 of 120 at 0.036, the single rule-off run being a sender writing 110.9 recovery messages a second on a bad link, slow rather than stalled. Every wedge classified in campaign `458b591` is the proof-chain mode: write-to-defer ratios 0.15, 0.25 and 0.21 against the in-process deadlock's lowest reading of 1.97, and the receive-queue drop counter fired zero times in all twenty runs |
