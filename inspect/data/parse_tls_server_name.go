...
// This function returns the server name found in a tls ClientHello message
//
// if serverName == "" then there was no server name in the ClientHello or not proper ClientHello
// getting ("", nil) means ignore
// getting ("", err) means an unexpected error occurred when parsing ClientHello
func getTlsServerName(tlsData []byte) (serverName string, err error) {
