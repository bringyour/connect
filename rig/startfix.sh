...
echo "STARTFIX START $(date -u +%T)"
for r in $(seq 1 $N); do
  if [ $((r % 2)) = 1 ]; then arms="A B"; else arms="B A"; fi
  for a in $arms; do
    if [ $a = A ]; then bin=urprovider.b-keys; dropin=""; else bin=urprovider.b-keys-fix; dropin=""; fi
    ...
    res=$(ssh -o BatchMode=yes $C "bash /root/ttfb-client.sh fx-$a-r$r" | tail -1)
    pm=$(ssh -o BatchMode=yes $P "md5sum /usr/local/bin/urprovider | cut -c1-12; tail -n +$((n0+1)) /var/log/urprovider.log | grep -c 'no pinned transport can connect'; tail -n +$((n0+1)) /var/log/urprovider.log | grep -c 'exit contract verification failed'" | tr '\n' ' ')
    echo "fx arm=$a r=$r | $res | provider md5/fixReleaseLogs/verifyExits: $pm"
  done
done
ssh -o BatchMode=yes $P "rm -f /etc/systemd/system/urprovider.service.d/exp.conf; systemctl daemon-reload"
echo "STARTFIX DONE $(date -u +%T)"
