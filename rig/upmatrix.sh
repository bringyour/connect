...
arms=(BETA UP UPnb MG)
echo "UPMATRIX $PFX START $(date -u +%T)"
for r in $(seq 1 $N); do
  for f in $FLOWS; do
    for k in 0 1 2 3; do
      a=${arms[$(( (k + r) % 4 ))]}
      case $a in
        ...
        UPnb) pb=up; bin=/tmp/urtun-up; ce="";;
        MG) pb=mg; bin=/tmp/urtun-mg; ce="HARNESS_MEMORY_BUDGET_MIB=384";;
      esac
      L=$PFX$f-$a-r$r
      grep -q "^$L " $S/upmatrix-$PFX.out 2>/dev/null && continue
...
echo "UPMATRIX $PFX DONE $(date -u +%T)"
