...
for r in $(seq 1 14); do
  ...
  if echo "$pd" | grep -q '^1 ' && [ "${g:-999}" -lt 600 ]; then
    ...
    scp -o BatchMode=yes -q $C:/tmp/sw-$L.goroutines $S/cli-gr-$L.txt 2>/dev/null; scp -o BatchMode=yes -q $C:/tmp/sw-$L.rate $S/cli-rate-$L.txt 2>/dev/null; scp -o BatchMode=yes -q $C:/tmp/sw-$L.ss1 $S/cli-ss1-$L.txt 2>/dev/null; scp -o BatchMode=yes -q $C:/tmp/sw-$L.ss2 $S/cli-ss2-$L.txt 2>/dev/null; scp -o BatchMode=yes -q $C:/tmp/sw-$L.nstat $S/cli-nstat-$L.txt 2>/dev/null
    echo "CAUGHT $L"; break
  fi
done
echo "CATCH2 DONE $(date -u +%T)"
